...
function makeStoreIcon(availability: StoreAvailability, highlighted: boolean): L.DivIcon {
...
export default function StoresMap({ stores, userLat, userLng, maxDistanceKm, hoveredStoreId }: Props) {
    const mappable = stores.filter((s) => s.latitude !== null && s.longitude !== null);
            ...
            {hasUser && (
                <>
                    ...
                    <Marker position={[userLat!, userLng!]} icon={USER_ICON}>
                        <Popup>
                            <span style={{ fontSize: '13px', fontWeight: 600 }}>Your location</span>
                        </Popup>
                    </Marker>
                    {maxDistanceKm && (
                        <Circle
            ...
            {mappable.map((store) => {
                ...
                return (
                    <Marker
                        ...
                        icon={makeStoreIcon(store.store_availability, highlighted)}
                        zIndexOffset={highlighted ? 1000 : 0}
                    >
                        <Popup>
                            <div style={{ minWidth: '160px', maxWidth: '200px' }}>
                                <p style={{ fontWeight: 600, fontSize: '13px', marginBottom: '3px' }}>
                                    {store.name}
                                </p>
                                {store.type && (
